...
//Vector<Thread> Tenenmy=new Vector<Thread>();	
  ddd(){
	  ...
	  //setContentPane(new jumsu(1000));
  
	  setContentPane(new mypan1());
  }
	...
	public class jumsu extends JPanel implements ActionListener{
      JPanel hl; JPanel h2; JPanel h3;
		JLabel jum;JLabel su; JLabel gru;
		JTextField name; JLabel su2; JButton fgd;JButton fgd2;
		int scored=0;
		jumsu(int scored){
			this.scored=scored;
			setSize(1000,1000);
		  ...
		  jum=new JLabel("이름:");
		 // jum.setSize(22,50);
		  jum.setFont(new Font(null,getFont().BOLD,40));
		  gru=new JLabel("결과");
		  gru.setFont(new Font(null,getFont().BOLD,90));
    	   hl=new JPanel(new FlowLayout());
    	   hl.add(gru);
    	   h2=new JPanel(new GridLayout(2,1));
    	   JPanel h22=new JPanel(new FlowLayout()); 
    	   name=new JTextField(9);
    	   name.setFont(new Font(null,getFont().BOLD,40));
    	   h22.add(jum);h22.add(name);
    	   JPanel h21=new JPanel(new FlowLayout());
    	   
    	   su=new JLabel("점수:"+scored);
    	   su.setFont(new Font(null,getFont().BOLD,40));
    	   h21.add(su);
    	   h2.add(h22);h2.add(h21);
    	   fgd=new JButton("기록"); fgd2=new JButton("취소");
    	   fgd.setFont(new Font(null,getFont().BOLD,40));
    	   fgd.addActionListener(this);
    	   
    	   fgd2.setFont(new Font(null,getFont().BOLD,40));
    	   fgd2.addActionListener(this);
    	   JPanel h31=new JPanel(new GridLayout(1,2));
    	   JPanel h32=new JPanel(new FlowLayout());
    	   JPanel h33=new JPanel(new FlowLayout());
    	   h33.add(fgd);h33.add(fgd2);
    	   h31.add(h32);h31.add(h33);
    	   h3=new JPanel(new GridLayout(1,1));
    	   h3.add(h31);
    	   add(hl);
    	   add(h2);
    	   add(h3);
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(arg0.getSource()==fgd) {
				textIOE G2=new textIOE();
				G2.textIOE2(name.getText(),scored);
				
				intro01 it = new intro01();
				it.display();
				dispose();
			}
			if(arg0.getSource()==fgd2) {
				intro01 it = new intro01();
				it.display();
				dispose();
...
//UI
 ...
 if(gunmode==1)
 {g.drawString("탄환:"+spround,670,30);g.drawString("모드:샷건", 830, 30);}
 else if(gunmode==0)
	 {g.drawString("탄환:무한",670,30);g.drawString("모드:권총", 830, 30);}
 else if(gunmode==2)
 {g.drawString("탄환:"+spround,670,30);g.drawString("모드:기관총", 830, 30);}
 else if(gunmode==3)
 {g.drawString("탄환:"+spround,670,30);g.drawString("모드:저격총", 830, 30);}

 
 g.drawString("체력:"+hp, 525, 30);g.drawString("적수 :"+enemyq.size(), 530, 60);
 
 if(gamemode==0&&hp<0)
 {g.drawString("게임을 져었드아 점수:"+score, 350,500);
  g.drawString("R:Game E:Record", 350,550);
 }
		...
		class enemyP implements Runnable{
			...
			public void run() {
				while(true)	{
					...
					enemyq.add(new enemyR(new Point(rand.nextDouble()*1000,1000,k1,0,0)));}
					else if(separ==1) {Random rand=new Random();
					//enenmy.add(new Point(1000,Math.random()*1000,k1,0,0));
					enemyq.add(new enemyR(new Point(1000,rand.nextDouble()*1000,k1,0,0)));}
					else if(separ==2) {Random rand=new Random();
						//enenmy.add(new Point(0,Math.random()*1000,k1,0,0));
						enemyq.add(new enemyR(new Point(0,rand.nextDouble()*1000,k1,0,0)));}
					else if(separ==3) {Random rand=new Random();
						//enenmy.add(new Point(Math.random()*1000,0,k1,0,0));
						enemyq.add(new enemyR(new Point(rand.nextDouble()*1000,0,k1,0,0)));}
					//int tv=Tenenmy.size();
